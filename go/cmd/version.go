...
func newVersionCmd(version string, buildTime string, gitCommit string) *cobra.Command {
	return &cobra.Command{
		Use:          "version",
		Short:        "telegram-send-go version",
		Args:         cobra.NoArgs,
		SilenceUsage: true,
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Println("telegram-send-go")
			fmt.Println("A Telegram upload watcher CLI.")
			fmt.Println("Author: dengqi935@gmail.com")
			fmt.Println("Github: https://github.com/nerdneilsfield/telegram-upload-watcher")
			fmt.Fprintf(cmd.OutOrStdout(), "telegram-send-go: %s\n", version)
